...
import { ExternalLink, Key, Loader2, Terminal as TerminalIcon, Check, X } from 'lucide-react';
import { useTranslations } from 'next-intl';

/**
 * Модалка настройки Codex CLI на устройстве.
 *   1. Install — `npm i -g @openai/codex` через exec на агенте
 *   2. API key — пользователь вставляет sk-..., мы пишем в systemd-override + ~/.bashrc
 *
 * Зеркалит GeminiSetupModal — отличается только подписями, ссылкой на API-keys
 * страницу OpenAI и форматом ключа (sk-...).
 */
export default function CodexSetupModal({
  deviceId,
  deviceName,
  alreadyInstalled,
  alreadyLoggedIn,
  ...
  const t = useTranslations('codexSetup');
  ...
  async function streamSse(url: string, body: Record<string, unknown>) {
    ...
    setBusy(true); setDone(false); setErr(null); setOutput('');
    try {
  ...
  async function runInstall() {
    await streamSse(`/api/devices/${deviceId}/codex-install`, {});
  }

  async function runSetKey() {
    if (!apiKey.trim()) { setErr(t('alertNoKey')); return; }
    await streamSse(`/api/devices/${deviceId}/codex-set-api-key`, { apiKey: apiKey.trim() });
    if (!err) setApiKey('');
  }
